...
def generate_content_from_template(
    ...
    if not os.path.isfile(template_file_path) or not os.access(
...
def generate_bes_from_template(
    template_dict, template_file_path=None, partials_path="."
):
    """This function has been renamed to generate_content_from_template"""
    template_dict = get_missing_bes_values(template_dict)
    return generate_content_from_template(
        template_dict, template_file_path, partials_path=partials_path
    )
...
def get_missing_bes_values(template_dict):
    ...
    if (
        "DownloadSize" not in template_dict
        and "prefetch" in template_dict
        and "size" in template_dict["prefetch"]
    ):
...
def main():
    ...
    template_file_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "TEMPLATE_TASK.bes.mustache"
    )
    template_dict = {
